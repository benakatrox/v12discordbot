...
if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(new Discord.MessageEmbed().setColor('RED') .setDescription(`Bu komutu kullanabilmek için **YONETICI** yetkisinin bulunduğun rolde olması gerekmektedir.`)).then(m => m.delete(({ timeout: 4000})));
 ...
 if(!rol) return message.channel.send(new Discord.MessageEmbed().setColor('RED') .setDescription(`Bu Özellik **Ayarlanmamış** Sıfırlayamazsın!`)).then(m => m.delete(({ timeout: 4000})));
 
 
  message.channel.send(new Discord.MessageEmbed().setColor('RED') .setDescription(`Otorol **MESAJI** Sıfırlandı!`)).then(m => m.delete(({ timeout: 4000})));
